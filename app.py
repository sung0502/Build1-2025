...
# ----------------------------
# Natural language parsing (lightweight)
# ----------------------------
WEEKDAYS = {
...
def handle_confirmation(user_text: str):
    ...
    if yn is None:
        assistant("Please reply with **yes** to save or **no** to discard.", stage_hint=True)
        return
    if yn is True:
...
def handle_plan_create(user_text: str, stay_in_stage: bool = False):
    ...
    if m.required_slots["time_date"] is None:
        assistant("Great. When should it happen? (e.g., *tomorrow 7-9pm*, *Fri 08:00-09:00*)", stage_hint=True); return
    if m.required_slots["duration"] is None:
        assistant("And the **duration**? (e.g., *90 minutes*, *1h 30m*)", stage_hint=True); return
...
def handle_plan_edit(user_text: str):
    ...
    if "delete" in t or "cancel" in t:
        ...
        if not matches: assistant("Which item should I delete? Tell me a few words from its title.", stage_hint=True); return
        if len(matches) > 1: assistant(f"I found multiple matches: {', '.join([m['title'] for m in matches])}. Which one?", stage_hint=True); return
        st.session_state.agenda = [x for x in st.session_state.agenda if x["id"] != matches[0]["id"]]
        assistant(f"Deleted **{matches[0]['title']}**. What else can I help you with?"); st.session_state.model.stage=None; return

    # move/reschedule/change time
    if any(k in t for k in ["move","reschedule","change","shift","delay"]):
        frag = extract_title_guess(user_text) or ""
        matches = find_items_by_title_fragment(frag) if frag else []
        if not matches: assistant("Which item should I move? Share a keyword from its title.", stage_hint=True); return
        if len(matches) > 1: assistant(f"I found multiple matches: {', '.join([m['title'] for m in matches])}. Which one?", stage_hint=True); return
        target = matches[0]
        d, start_t, end_t, dur = normalize_times(user_text, now_tz())
        if not (d and start_t and end_t): assistant("Where should I move it? Say like: *tomorrow 7-8am* or *Fri 14:00-15:00*.", stage_hint=True); return
        target["date"] = d.isoformat(); target["start"] = start_t.strftime("%H:%M"); target["end"] = end_t.strftime("%H:%M")
        if dur: target["duration_min"] = dur
        assistant(f"Updated **{target['title']}** → {d.strftime('%a, %b %d')} {target['start']}-{target['end']}."); st.session_state.model.stage=None; return

    # rename
    if "rename" in t:
        m = re.search(r'rename\s+(.+?)\s+to\s+(.+)$', user_text, re.I)
        if not m: assistant("Tell me: **rename [old words] to [new title]**.", stage_hint=True); return
        ...
        if len(matches) > 1: assistant(f"Multiple matches: {', '.join([m['title'] for m in matches])}. Which one?", stage_hint=True); return
        matches[0]["title"] = new_title
        assistant(f"Renamed to **{new_title}**."); st.session_state.model.stage=None; return
...
def handle_plan_check(user_text: str):
    ...
    st.markdown("Quick actions: try <span class='kbd'>move …</span>, <span class='kbd'>delete …</span>, or use the buttons.", unsafe_allow_html=True)
...
def route_to_stage(user_text: str):
    ...
    if conf < 0.6:
        assistant("I think you're trying to schedule something. Could you share the **title**, **time/date**, and **duration**?", stage_hint=True); return
